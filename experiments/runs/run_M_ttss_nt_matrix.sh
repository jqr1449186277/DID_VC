...
CSV
  cat > "$AGG_CSV" <<'CSV'
group_name,group_order,point_order,n,t,point_key,leaked_indexes,challenge_count,max_queries,recover_status,recover_runs,recover_pass_count,recover_pass_rate,avg_recover_setup_ms,avg_recover_ms,avg_rotate_ms,avg_share_count_used,trace_status,trace_runs,trace_pass_count,trace_pass_rate,avg_trace_setup_ms,avg_load_box_ms,avg_oracle_probe_ms,avg_trace_run_ms,avg_trace_verify_ms,avg_publish_ms,avg_trace_query_count,avg_accused_set_size,point_dir,note
CSV
}
...
run_point(){
  local group_name="$1" group_order="$2" point_order="$3" n="$4" t="$5"
  local point_key="n${n}_t${t}"
  local point_dir="$OUT_DIR/points/$point_key"
  local reuse_from=""
  local note=""

  if [[ -d "$point_dir" && "$RERUN_DUPLICATE_POINTS" != "1" ]]; then
    reuse_from="$point_dir"
    log "reuse point $point_key for group=$group_name"
  else
    mkdir -p "$point_dir"
    ...
    log "run point group=$group_name n=$n t=$t leaked=$leaked_indexes challenge_count=$challenge_count"
    ...
    if [[ "$ENABLE_TRACE" == "1" ]]; then
      ...
      if ROOT="$ROOT" BIN="$BIN" OUT_DIR="$trace_dir" OUT_BASE="$OUT_BASE" BB="$BB" PIRATE="$PIRATE" TOKEN="$TOKEN" \
        ...
        note="${note}trace_failed;"
      fi
    fi
  fi

  local subset_urls leaked_indexes challenge_count
  subset_urls="$(subset_committee_urls "$n")" || die "subset_committee_urls failed for summarize $point_key"
  leaked_indexes="$(choose_leaked_indexes "$n" "$t")" || die "choose_leaked_indexes failed for summarize $point_key"
  challenge_count="$(compute_challenge_count "$n" "$t")" || die "compute_challenge_count failed for summarize $point_key"
  ...
  if [[ "$ENABLE_RECOVER" == "1" && -f "$point_dir/recover/summary.csv" ]]; then
    local rjson
    rjson="$(summarize_csv "$point_dir/recover/summary.csv" recover)"
  ...
  elif [[ "$ENABLE_RECOVER" == "1" ]]; then
    recover_status="fail"
  fi

  if [[ "$ENABLE_TRACE" == "1" && -f "$point_dir/trace/summary.csv" ]]; then
    local tjson
    tjson="$(summarize_csv "$point_dir/trace/summary.csv" trace)"
...
run_group(){
  local group_name="$1" group_order="$2" pair_list="$3"
  local point_order=1
  ...
  for pair in "${pairs[@]}"; do
    ...
    run_point "$group_name" "$group_order" "$point_order" "$n" "$t"
...
main(){
  ...
  write_headers
  run_group fixed_t4 1 "$GROUP_FIXED_T4"
  run_group fixed_n6 2 "$GROUP_FIXED_N6"
  run_group selected_points 3 "$GROUP_SELECTED"
  write_group_tables
