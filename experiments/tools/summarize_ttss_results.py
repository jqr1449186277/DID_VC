...
def percentile(xs, q):
    ...
    if len(xs)==1:
        return round(xs[0],3)
    idx=(len(xs)-1)*q
    lo=int(idx); hi=min(lo+1, len(xs)-1); frac=idx-lo
...
def latest_dir(base: Path, suffix: str) -> Optional[Path]:
    dirs=[p for p in base.glob(f'*_{suffix}') if p.is_dir()]
...
def summarize_trace_dir(d: Path) -> Dict[str,Any]:
    summary_rows=read_csv(d/'summary.csv'); attempt_rows=read_csv(d/'attempts.csv')
    runs=len(summary_rows); pass_rows=[r for r in summary_rows if truthy(r.get('pass'))]; pass_count=len(pass_rows)
    retry_total=max(0,len(attempt_rows)-runs)
...
def resolve_suite_components(suite_dir: Path):
    rd=suite_dir/'J_ttss_recover_batch'; td=suite_dir/'K_ttss_trace_batch'; sd=suite_dir/'L_ttss_scale'
    return (rd if (rd/'summary.csv').exists() else None, td if (td/'summary.csv').exists() else None, sd if (sd/'aggregate.csv').exists() else None)

def collect_rows_from_suite(suite_dir: Path) -> List[Dict[str,Any]]:
    rows=[]; rd,td,sd=resolve_suite_components(suite_dir)
    if rd: rows.append(summarize_recover_dir(rd))
    if td: rows.append(summarize_trace_dir(td))
    if sd: rows.extend(summarize_scale_dir(sd))
    return rows
...
def main():
    ...
    ap.add_argument('--recover-dir'); ap.add_argument('--trace-dir'); ap.add_argument('--scale-dir'); ap.add_argument('--out-dir')
    ap.add_argument('--all', action='store_true')
    args=ap.parse_args()
    root=Path(args.root).expanduser(); results=root/'results'; rows=[]
    if args.all:
        for d in sorted(results.glob('*_J_ttss_recover_batch')):
            if d.is_dir() and (d/'summary.csv').exists(): rows.append(summarize_recover_dir(d))
        for d in sorted(results.glob('*_K_ttss_trace_batch')):
            if d.is_dir() and (d/'summary.csv').exists(): rows.append(summarize_trace_dir(d))
        for d in sorted(results.glob('*_L_ttss_scale')):
            if d.is_dir() and (d/'aggregate.csv').exists(): rows.extend(summarize_scale_dir(d))
        for s in sorted(results.glob('*_JKL_suite')):
            if is_suite_dir(s): rows.extend(collect_rows_from_suite(s))
    else:
        ...
        if suite_dir and is_suite_dir(suite_dir):
            ...
            default_out=results/(datetime.now().strftime('%Y%m%d_%H%M%S')+'_TTSS_overall_report')
            out_dir=Path(args.out_dir).expanduser() if args.out_dir else default_out
        else:
            rd=Path(args.recover_dir).expanduser() if args.recover_dir else latest_dir(results,'J_ttss_recover_batch')
            td=Path(args.trace_dir).expanduser() if args.trace_dir else latest_dir(results,'K_ttss_trace_batch')
            sd=Path(args.scale_dir).expanduser() if args.scale_dir else latest_dir(results,'L_ttss_scale')
            if rd and (rd/'summary.csv').exists(): rows.append(summarize_recover_dir(rd))
            if td and (td/'summary.csv').exists(): rows.append(summarize_trace_dir(td))
            if sd and (sd/'aggregate.csv').exists(): rows.extend(summarize_scale_dir(sd))
            default_out=results/(datetime.now().strftime('%Y%m%d_%H%M%S')+'_TTSS_overall_report')
            out_dir=Path(args.out_dir).expanduser() if args.out_dir else default_out
    if not rows:
